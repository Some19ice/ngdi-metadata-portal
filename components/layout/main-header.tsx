...
export default function MainHeader() {
  ...
  return (
    <div className="sticky top-0 z-50 flex flex-col px-4 pt-4">
      {/* ===== UNIFIED BANNER & NAVIGATION CONTAINER ===== */}
      <div className="relative overflow-hidden rounded-2xl border border-border/40 shadow-xl">
        {/* Shared animated background layers for entire header */}
        <div className="absolute inset-0 pointer-events-none">
          {/* Base gradient layer */}
          <div className="absolute inset-0 bg-gradient-to-br from-slate-900/95 via-slate-800/95 to-slate-900/95 dark:from-slate-950/95 dark:via-slate-900/95 dark:to-slate-950/95"></div>

          {/* Animated gradient orbs */}
          <div className="absolute -top-20 left-1/4 w-96 h-96 bg-blue-500/15 rounded-full blur-3xl animate-pulse"></div>
          <div
            className="absolute -top-20 right-1/4 w-96 h-96 bg-green-500/15 rounded-full blur-3xl animate-pulse"
        ...
        {/* ===== BANNER SECTION ===== */}
        <div
          className="relative h-[var(--banner-height)] flex items-center text-white overflow-hidden"
          role="banner"
        ...
        <header
          className={cn(
            "relative w-full h-[var(--header-height)] transition-all duration-300 ease-in-out",
            isScrolled
